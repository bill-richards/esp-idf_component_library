...
namespace gsdc_cppiic
{
    IIC::IIC(i2c_port_t port, size_t slv_rx_buf_len, size_t slv_tx_buf_len, int intr_alloc_flags)
    {
        _port = port;
    ...
    esp_err_t IIC::InitMaster(int sda_io_num, int scl_io_num, uint32_t clk_speed, bool sda_pullup_en, bool scl_pullup_en, uint32_t clk_flags)
    {
    ...
    uint8_t IIC::ReadRegister(uint8_t dev_addr, uint8_t reg_addr)
    {
        uint8_t rxBuf{};
    ...
    esp_err_t IIC::WriteRegister(uint8_t dev_addr, uint8_t reg_addr, uint8_t txData)
    {
        const uint8_t txBuf[2] {reg_addr, txData};
        return i2c_master_write_to_device(_port, dev_addr, txBuf, 2, pdMS_TO_TICKS(1000));
    }

    esp_err_t IIC::ReadRegisterMultipleBytes(uint8_t dev_addr, uint8_t reg_addr, uint8_t *rx_data, int length)
    {
        return i2c_master_write_read_device(_port, dev_addr, &reg_addr, 1, rx_data, length, pdMS_TO_TICKS(1000));
    }

    esp_err_t IIC::WriteRegisterMultipleBytes(uint8_t dev_addr, uint8_t reg_addr, uint8_t *tx_data, int length)
    {
        esp_err_t status = ESP_OK;
