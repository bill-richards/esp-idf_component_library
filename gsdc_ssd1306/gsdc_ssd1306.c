...
void ssd1306_init(SSD1306_t * dev, int width, int height)
{
	...
	// } else {
		ESP_LOGI(TAG, "calling gsdc_ssd1306_i2c_init");
		gsdc_ssd1306_i2c_init(dev, width, height);
	// }
...
void ssd1306_show_buffer(SSD1306_t * dev)
{
	if (dev->_address == SPIAddress) {
		...
		for (int page=0; page<dev->_pages;page++) {
			gsdc_ssd1306_i2c_display_image(dev, page, 0, dev->_page[page]._segs, dev->_width);
		}
	}
}
...
void ssd1306_display_image(SSD1306_t * dev, int page, int seg, uint8_t * images, int width)
{
	if (dev->_address == SPIAddress) {
		...
		gsdc_ssd1306_i2c_display_image(dev, page, seg, images, width);
	}
...
void ssd1306_display_text(SSD1306_t * dev, int page, char * text, int text_len, bool invert)
{
	...
	for (uint8_t i = 0; i < _text_len; i++) {
		...
		if (dev->_address == SPIAddress) {
			...
			gsdc_ssd1306_i2c_display_image(dev, page, seg, image, 8);
		}
#endif
		seg = seg + 8;
	}
	...
	for (uint8_t nn = 0; nn < _text_len; nn++) {
		...
		for (uint8_t yy = 0; yy < 3; yy++)	{ // for each group of 8 pixels high (y-direction)
			...
			if (dev->_address == SPIAddress) {
				...
				gsdc_ssd1306_i2c_display_image(dev, page+yy, seg, image, 24);
			}
...
void ssd1306_contrast(SSD1306_t * dev, int contrast)
{
	if (dev->_address == SPIAddress) {
		...
		gsdc_ssd1306_i2c_contrast(dev, contrast);
	}
}
...
void ssd1306_scroll_text(SSD1306_t * dev, char * text, int text_len, bool invert)
{
	...
	if (dev->_address == SPIAddress) {
		...
		func = gsdc_ssd1306_i2c_display_image;
	}
...
void ssd1306_hardware_scroll(SSD1306_t * dev, ssd1306_scroll_type_t scroll)
{
	if (dev->_address == SPIAddress) {
		...
		gsdc_ssd1306_i2c_hardware_scroll(dev, scroll);
	}
}
	...
	if (delay >= 0) {
		for (int page=0;page<dev->_pages;page++) {
			if (dev->_address == SPIAddress) {
				...
				gsdc_ssd1306_i2c_display_image(dev, page, 0, dev->_page[page]._segs, 128);
			}
