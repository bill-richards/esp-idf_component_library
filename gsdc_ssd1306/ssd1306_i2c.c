...
{
	gsdc_ssd1306_i2c_master_init_p(dev, sda, scl, reset, _i2cPort, _i2cClockFrequency);
}
...
void gsdc_ssd1306_i2c_init(SSD1306_t * dev, int width, int height) {
	...
	esp_err_t espRc = i2c_master_cmd_begin(_i2cPort, cmd, 10/portTICK_PERIOD_MS);
	if (espRc == ESP_OK) {
...
void gsdc_ssd1306_i2c_hardware_scroll(SSD1306_t * dev, ssd1306_scroll_type_t scroll) {
	esp_err_t espRc;
	...
	i2c_master_stop(cmd);
	espRc = i2c_master_cmd_begin(_i2cPort, cmd, 10/portTICK_PERIOD_MS);
	if (espRc == ESP_OK) {
